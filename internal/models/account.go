...
func (a *Account) GetUserTFORBalance(o *gorm.DB, uid uint) (resp.AccountInfoResp, error) {
	...
	rows := o.Raw(fmt.Sprintf("SELECT acc.currency_id,(acc.balance-acc.blocked_balance) as balance,cur.symbol,cur.decimals,cur.name from %s cur LEFT JOIN %s acc on acc.currency_id = cur.id where acc.uid = ? AND cur.symbol = ?", GetCurrencyTable(), GetAccountTable()), uid, "TFOR").Row()
	err := rows.Scan(&data.CurrencyId, &data.Balance, &data.Symbol, &data.Decimals, &data.Name)
	return data, err
}
