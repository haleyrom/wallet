...
func (c *CompanyStream) GetList(o *gorm.DB, page, pageSize, start_time, end_timer int, keyword string) (resp.CompanyStreamListResp, error) {
	...
	sql := fmt.Sprintf("SELECT detail.address,detail.id as id,user.id as uid,user.name,user.email,detail.income,detail.spend,detail.balance, detail.last_balance, currency.symbol,detail.updated_at,detail.order_id FROM %s detail LEFT JOIN %s user ON detail.uid = user.id LEFT JOIN %s account ON detail.account_id = account.id LEFT JOIN %s currency ON currency.id = account.currency_id where detail.id > 0 AND detail.code = '%s' ", GetCompanyStreamTable(), GetUserTable(), GetAccountTable(), GetCurrencyTable(), c.Code)
