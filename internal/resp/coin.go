...
type ReadCoinListResp struct {
	...
	MinDeposit        string `json:"min_deposit"`         // 最小充值金额，小于该金额不入账
	MinWithdrawal     string `json:"min_withdrawal"`      // 小于该金额不能提
	WithdrawalFee     string `json:"withdrawal_fee"`      // 提现手续费
...
type ReadOrderSymbolByCoinResp struct {
	...
	MinDeposit string `json:"min_deposit"` // 最小提币数量
}
